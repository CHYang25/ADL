...
def parse_args():
    ...
    parser.add_argument(
        "--context_file", type=str, default=None, help="A json file containing the context data."
    )
    parser.add_argument(
        "--max_seq_length",
        type=int,
...
def main():
    ...
    # Setup for exact match trace
    train_examples = raw_datasets["train"]

    # for learning curve plot
    loss_plt = []
    exact_match_list_plt = []
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(active_dataloader):
            with accelerator.accumulate(model):
                ...
                # for loss curve
                if step % 20 == 19:
                    loss_plt.append(float(loss.detach().float()))

            model.eval()                
            # for exact match curve
            with torch.no_grad():
                outputs = model(**batch)
            ...
            if step % 800 == 799: # check the average exact match in every 200 steps, also the loss
                
                # for exact match evalutation
                segment_examples = train_examples.select(range(step-799,step+1))
                with accelerator.main_process_first():
                    segment_dataset = segment_examples.map(
                        prepare_validation_features,
                        batched=True,
                        num_proc=args.preprocessing_num_workers,
                        remove_columns=column_names,
                ...
                start_logits_concat = create_and_fill_np_array(all_start_logits, segment_dataset, max_len)
                end_logits_concat = create_and_fill_np_array(all_end_logits, segment_dataset, max_len)

                # clear the list of numpy arrays
                all_start_logits = []
                all_end_logits = []

                outputs_numpy = (start_logits_concat, end_logits_concat)
                train_prediction = post_processing_function(segment_examples, segment_dataset, outputs_numpy)
                train_metric = metric.compute(predictions=train_prediction.predictions, references=train_prediction.label_ids)

                exact_match_list_plt.append(train_metric["exact_match"])
                print(f"\n{train_metric}\n")
    ...
    if args.output_dir is not None:
        ...
        if accelerator.is_main_process:
            ...
            with open("log.file", "w") as f:
                ...
                f.write("\n")
                f.write(str(exact_match_list_plt))
            
            # save the figure
            plt.plot(loss_plt)
            plt.suptitle("Loss learning curve")
            plt.savefig("loss_curve.png")

            # clear the previous figure
            plt.clf()
            plt.plot(exact_match_list_plt)
            plt.suptitle("EM learning curve")
