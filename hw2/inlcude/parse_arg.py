...
def parse_args():
    ...
    if args.train_file is None or args.validation_file is None or args.result_file is None:
        ...
        assert extension is "jsonl", "train_file should be a jsonl file"
        extension = args.validation_file.split(".")[-1] # to see what extension the file is
        assert extension is "jsonl", "validation_file should be a jsonl file"
        extension = args.result_file.split(".")[-1] # to see what extension the file is
        assert extension is "jsonl", "result_file should be a jsonl file"
        # the pre-trained model should be google/mt5-small
        assert args.model_name_or_path is "google/mt5-small", "the pre-trained model should be google/mt5-small"
    return args
